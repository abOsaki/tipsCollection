...
function packageSearch($param) {
    ...
    $sql = "SELECT `{$maindb}`.`{$tablename}`.`id` AS Id," .
        ...
        "`{$maindb}`.`{$tablename}`.`contents` AS Contents, " .
...
function workbookDelete($param){
    $model = new dbConnect(ANAUME_TABLE);
    $pdo = $model -> connectInfo();
    $tablename = "package";

    $sql = "SELECT `id`, `author`, `contents` FROM `{$tablename}` ";
    ...
    if (empty($array)) {
        $result["result"] = 'error';
    } else if ($array["author"] != $_SESSION["USERID"]) {
        $result["result"] = "他の方が作成しているため削除できません";
    } else {
        //ここでquestionのbelongCountを調整する処理を入れる
        ...
        $sql = "DELETE FROM `{$tablename}` WHERE `id` = :id AND `author` = :author ";
        $stmt = $pdo -> prepare($sql);
        $stmt -> bindValue(':id', $param["id"], PDO::PARAM_STR);
        $stmt -> bindValue(':author', $_SESSION["USERID"], PDO::PARAM_STR);
        $stmt -> execute(null);
        $result["result"] = "問題集を削除いたしました";
        $result["deleteID"] = $param["id"];
    }
    return $result;
}
...
if ($req["command"] == "packageSearch"){
    $res = packageSearch($req["param"]);
} else if ($req["command"] == "packageDelete"){
    $res = workbookDelete($req["param"]);
} else {
    $res = null;
}
