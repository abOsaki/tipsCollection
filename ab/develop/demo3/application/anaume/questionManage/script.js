...
var fn = (function () {
    ...
    return {
            ...
            $.ajax({
                ...
                if (data.length > 0) {
                    for (i = 0; i < data.length; i += 1) {
                        ...
                        tr +=
                            '<tr>' +
                            '<td><input type="radio" value="' + obj.Id + '" name="questionID" class="questionID" /></td>' +
        ...
        questionDelete: function () {
            var sendData = {
                command: 'questionDelete',
                param: {}
            };
            sendData.param.id = $("input:radio[name='questionID']:checked").val();
            ...
            $.ajax({
                ...
                if (data.result) {
                    ...
                    if (data.deleteID) {
                        $('.questionID[value="' + data.deleteID + '"]').closest('tr').remove();
                    }
                }
            }).always(function (data) {
                //console.log(data);
...
$(function () {
    ...
    $(document).on('change', '#category', function () {
        ...
        //削除処理
        adminfn.alertDialog("注意", "選択した設問を削除します。<br>本当によろしいですか？", fn.questionDelete);
        ...
        if (!$("input:radio[name='questionID']:checked").val()) {
            //チェックされていないときの処理
            adminfn.alertDialog("", "設問を選択してください");
        } else {
            var linkId = $("input:radio[name='questionID']:checked").val();
            location.href = "./../question/?q=" + linkId;
        }
