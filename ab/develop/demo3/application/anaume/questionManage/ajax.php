...
function questionDelete($param){
    $model = new dbConnect(ANAUME_TABLE);
    $pdo = $model -> connectInfo();
    $tablename = "question";
    
    $sql = "SELECT `id`, `author`, `belongCount` FROM `{$tablename}` WHERE `id` = :id ";
    $stmt = $pdo -> prepare($sql);
    ...
    if (empty($array)) {
        $result["result"] = 'error';
    } else if ($array["belongCount"] > 0) {
        $result["result"] = $array["belongCount"] . "つの問題集で使われているため削除できません";
...
$res = "";
if ($req["command"] == "questionSearch"){
    $res = questionSearch($req["param"]);
}else if ($req["command"] == "questionDelete"){
    $res = questionDelete($req["param"]);
}else{
    $res = null;
}
