...
(function(window) {
  ...
  return {
    ...
    m: [], //for plugins
    setXValueColorDictionary: function(pvContainValue,pvColor){
        if(!xValueColorDictionary){
            xValueColorDictionary = new Object();
        }
        xValueColorDictionary[pvContainValue] = pvColor;
    },
    ...
    preProcessing: function (op, callback) {
      ...
      */
      ...
      //カスタム項目
      this.yAxisVal = op.config.yAxisVal || [];
    ...
    drawXscale: function (left, count){
      ...
      if(this.type !== 'pie'){
        ...
        //xValueColorDictionaryのセット
        if(xValueColorDictionary){
            for (var keyString in xValueColorDictionary) {
                //valがその値を含む
                if(val[val.length - 1] == keyString){
                    val = val.substr( 0, val.length-1 ) ;
                    xScaleColor = xValueColorDictionary[keyString];
                }
            }
        }
    ...
    drawYscale: function(top, count){
      ...
      if(this.type === 'stacked%'){
        ...
        val = this.yAxisVal[count] || this.util.addComma(this, this.wkYScale, yScaleDecimal);
        percent = (this.yScalePercent==='yes')?( ' ('+ Math.round(count * 100/this.axisXLen)  +'%)' ):'';
      }
      this.wkYScaleStr = val + percent;
   ...
   // demo: 'http://ccchart.com/test/base/base-v1.06-b7.htm',
    ...
    _css_arc: function(op){
      ...
      function showTooltip(id, e, op){
        ...
        if(dateArray[1]){
            dateArray[1] = (dateArray[1] + '0').slice(0, 2);
        } else {
            dateArray[1] = '00';
        }
